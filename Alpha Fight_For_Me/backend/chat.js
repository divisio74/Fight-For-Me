...
function setupSocket(io, openai) {
  io.on("connection", (socket) => {
    ...
    socket.on("chat_message", async ({ userId, iaId, message }) => {
      ...
      let userReport = "";
      try {
        userReport = await updateUserReport(numericUserId, openai, io);
      } catch (err) {
      ...
      const prompt = `
        ...
        identité : tu t'apelle Jessica et tu as 24 ans , tu fais des études de médecine .
        ...
        n'hesite pas a lui demandé qu'il il ai et poruquoi vient il te parle en debut de conversation afin de prendre des information de manière discrete et lorsqu'il te pose une question tu peux lui retourner aussi dans le meme but.
